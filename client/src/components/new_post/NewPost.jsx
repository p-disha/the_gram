...
export default function NewPost({ pageType }) {
  ...
  const [tags, setTags] = useState(""); // New state for tags (input as a comma-separated string)
  ...
  function handleLocationChange(e) {
    setLocation(e.target.value); // Handle location input change
  }

  function handleTagsChange(e) {
    setTags(e.target.value); // Handle tags input change
  }
  ...
  async function handleFormSubmit(e) {
    ...
    const nPost = {
      userId: user._id,
      desc: desc,
      location: location, // Include location in the post data
      tags: tags.split(',').map((tag) => tag.trim()), // Convert comma-separated string to an array of trimmed strings
      img: file
    };
